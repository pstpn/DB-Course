...
\begin{table}[h!]
	\begin{center}
		...
		\begin{tabularx}{\textwidth}{|X|X|X|}
			...
			\texttt{checkpoint\_id} & \texttt{integer} & Идентификатор КПП, через который был проход \\ \hline
			\texttt{document\_id} & \texttt{integer} & Идентификатор документа, по которому был проход \\ \hline
...
\begin{table}[h!]
	\begin{center}
		...
		\begin{tabularx}{\textwidth}{|X|X|X|}
			...
			\texttt{phone\_number} & \texttt{text} & Номер телефона КПП \\ \hline
		\end{tabularx}
	\end{center}
\end{table}
